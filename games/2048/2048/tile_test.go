...
func cellsToTiles(cells []int, size int) map[*2048.Tile]struct{} {
	tiles := map[*2048.Tile]struct{}{}
	for j := 0; j < size; j++ {
		for i := 0; i < size; i++ {
			c := cells[i+j*size]
			if c == 0 {
...
func tilesToCells(tiles map[*2048.Tile]struct{}, size int) ([]int, []int) {
...
func TestMoveTiles(t *testing.T) {
	...
	for _, test := range testCases {
		...
		moved := 2048.MoveTiles(tiles, size, test.Dir)
